...
- [x] **4.13** *(upgrade available: closed-form rho*_q derived in notes/model_derivations.tex — swap into 3.3 when convenient)* (memo M1) Real-rate result: state propositions for e; add the ρ*(q) = 4.93 > ρ*(e) = 3.96 result with the mechanical-CPI intuition, the Balassa–Samuelson sentence, and the non-tradables-inert-for-the-mechanism point; report Δq alongside Δe in the tariff experiments.
