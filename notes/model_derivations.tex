...
Let $\widetilde J$ be the full $N \times N$ Jacobian before dropping country $A$. Homogeneity (trade balances depend only on relative currency values) and Walras' law give the identities
\begin{equation}
...
By \eqref{eq:dp}, prices enter \eqref{eq:dindex}--\eqref{eq:dtb} only through differences $\nu_j - \nu_i$, so adding a common constant to every $\nu_i$ (including $\nu_A$) leaves each $d\,\mathrm{TB}_i$ unchanged: $\sum_l \widetilde J_{il} = 0$ for all $i$. Summing \eqref{eq:tb} over $i$ gives $\sum_i \mathrm{TB}_i \equiv 0$ identically, so $\sum_i \widetilde J_{il} = 0$ for all $l$. Under Assumption \ref{as:gs}, $\widetilde J_{il} \geq 0$ for $l \neq i$, so $\widetilde J_{ii} = -\sum_{l \neq i} \widetilde J_{il} \leq 0$, and deleting row and column $A$ gives \eqref{eq:diagdom}, with strict inequality in every row $i$ with $\widetilde J_{iA} > 0$.
