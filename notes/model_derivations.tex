...
This section develops the expression for $N$-country exchange-rate adjustments and its implications for a general tariff vector under symmetry and the MML condition. We begin with the bilateral case and then extend the same logic to a general differentiable trade system.
...
Stack the log exchange rates as $\nu\equiv(\nu_j)_{j\neq A}$, with $\nu_j\equiv\log e_{Aj}$, and the bilateral tariff changes as $d\boldsymbol{\tau}\equiv(dt_{jk})_{j\neq k}$. Linearizing the $N-1$ trade-balance conditions gives
\begin{equation}
J\,d\nu+G\,d\boldsymbol{\tau}=0,
\qquad
J_{il}\equiv\frac{\partial\,\mathrm{TB}_i}{\partial\nu_l},
\qquad
G_{i,(jk)}\equiv\frac{\partial\,\mathrm{TB}_i}{\partial t_{jk}}\bigg|_{\nu\ \mathrm{fixed}} .
\label{eq:JFdef}
\end{equation}

Now separate a general tariff change into its direction and scale. Let $\boldsymbol{a}$ denote the direction of the tariff change, normalized so that $\|\boldsymbol{a}\|_2=1$, and let $ds$ denote its scale, so that
\[
