...
In the standard two-country setting, a tariff that improves the tariffing country's trade balance at unchanged exchange rates appreciates its currency under the Marshall--Lerner condition. With more than two countries, a tariff on one trading partner also reallocates expenditure across foreign suppliers, so bilateral exchange rates need not all move in the same direction. Such third-country effects are the subject of the customs-union and preferential-tariff literature on the terms of trade (Viner, 1950; Mundell, 1964; Kemp and Wan, 1976); this paper connects them to the two-country tariff and exchange-rate result.

This paper characterizes the exchange-rate effects of tariffs in an $N$-country model. The general log-linearized equilibrium response is $(-J)^{-1}F$, where $F$ gives the trade-balance effects of a tariff at unchanged exchange rates and $J$ is the Jacobian of trade balances with respect to exchange rates. Under a multilateral Marshall--Lerner (MML) condition on $J$, this representation separates the trade-balance effects of the tariff from the exchange-rate adjustment required to restore equilibrium.

Under partner symmetry, the exchange-rate response to any tariff vector can be decomposed into average protection and relative treatment. Section~\ref{sec:general} derives this decomposition without imposing a specific model of preferences or trade. It shows that average protection determines the tariffing country's nominal effective exchange rate (NEER), while relative treatment determines how its bilateral exchange rates differ across partners. Section~\ref{sec:model} then derives these responses in a nested-CES model and shows how they depend on substitution elasticities, expenditure shares, and the number of countries. For a unilateral tariff, the tariffing country appreciates against the targeted partner and its NEER appreciates, but its exchange rate against an untariffed bystander can move in either direction. A bystander depreciation occurs only when substitution across foreign import origins is sufficiently strong relative to the other effects of the tariff (Section~\ref{sec:unilateral}). Section~\ref{sec:wars} considers retaliatory and preferential tariffs, including a comparison with the standard competing-exporter benchmark. Proofs are collected in Appendix~\ref{app:proofs}.
