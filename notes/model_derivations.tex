...
\begin{proposition}[Two-country benchmark]
...
\noindent Figure~\ref{fig:twocountry} displays \eqref{eq:2c-collected} and \eqref{eq:2c-slope}. Appendix~\ref{app:cd} gives the exact finite-tariff Cobb--Douglas solution.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{fig1_two_country.pdf}
\caption{Two-country equilibrium adjustment. The schedule is $d\,\mathrm{TB}_A=mD_2\,\nu_B+m\rho_1^*\,d\tau$ from \eqref{eq:2c-collected}; its slope $mD_2=m(\varepsilon_X+\varepsilon_M-1)$ is positive exactly under the Marshall--Lerner condition. At unchanged exchange rates the tariff shifts the schedule up by $m\rho_1^*\,d\tau>0$ (open square). The currency then appreciates along the new schedule until balance is restored at $\nu_B^{\ast}=-(\rho_1^*/D_2)\,d\tau$. Drawn at $\alpha_D=0.7$, $\alpha_T=0.6$, $\eta=1.5$.}
...
The first central result collects the general adjustment representation, a primitive sufficient condition for MML, and the resulting sign rule.

\begin{theorem}[General multilateral equilibrium adjustment]
Consider any differentiable multilateral trade environment with $N-1$ independent trade-balance conditions and matrices $J$ and $G$ as in \eqref{eq:JFdef}.
\begin{enumerate}
\item[(i)] If MML \eqref{eq:invpos} holds, the equilibrium response to a tariff change $dt$ is uniquely determined and given by
\begin{equation}
d\nu = (-J)^{-1}G\,dt ,
\label{eq:adjust}
\end{equation}
with $(-J)^{-1}\geq0$ elementwise and $\det(-J)>0$.
\item[(ii)] Under Assumption~\ref{as:gs}, with $J$ irreducible and $\widetilde J_{iA} > 0$ for some $i$, MML \eqref{eq:invpos} holds; equivalently, $\operatorname{Re}\lambda(J)<0$.
\item[(iii)] For a tariff experiment $t=a\,d\tau$ with $F\equiv Ga$, each bilateral response satisfies
\begin{equation}
...
\end{equation*}
The matrix therefore acts separately on average protection and relative treatment. The corresponding eigenvalues are $a+(N-1)b$ and $a$. Since relative treatment sums to zero, it also has no effect on the equally weighted average of the bilateral exchange rates.
...
\noindent Lemma~\ref{lem:exch} separates the exchange-rate response into an average-protection component and a relative-treatment component. Average protection $\bar t$ determines the common movement in $A$'s bilateral exchange rates and hence its effective exchange rate. Relative treatment $\tilde t_j$ determines how the bilateral rate against partner $j$ differs from that common movement. Section~\ref{sec:symmetric} computes these responses for the nested-CES model.
...
\begin{theorem}[Average protection and relative treatment]
At the symmetric benchmark with $N\geq3$ and $D_N>0$, decompose the tariff vector imposed by $A$ as in \eqref{eq:t-split-scalar}, $t_j=\bar t+(t_j-\bar t)$. The equilibrium exchange-rate response decomposes correspondingly:
\begin{equation}
\nu_j=\lambda_S\,\bar t+\lambda_D\,(t_j-\bar t),
\label{eq:scale-disc}
\end{equation}
where $\lambda_S$ and $\lambda_D$ are given in \eqref{eq:eigen} and both are negative. Consequently,
\begin{enumerate}
\item[(i)] average protection alone determines the effective exchange rate,
\begin{equation}
d\nu_A^E=\lambda_S\,\bar t ;
\label{eq:neer-scale}
\end{equation}
\item[(ii)] relative treatment alone determines the dispersion of bilateral rates around it,
\begin{equation}
\nu_j-d\nu_A^E=\lambda_D\,(t_j-\bar t) ;
\label{eq:disp-disc}
\end{equation}
\item[(iii)] relative treatment alone determines the cross rates among $A$'s partners: for any $j,k\neq A$,
\begin{equation}
...
\end{equation*}
Substituting into Theorem~\ref{res:scaledisc} gives the canonical unilateral-tariff result.

\begin{theorem}[Unilateral tariff]
At the symmetric benchmark with $N\geq3$ and $D_N>0$, let $t=d\tau\,\mathbf{e}_B$ with $d\tau>0$. Then
\begin{enumerate}
\item[(i)] $A$ appreciates against the target,
\begin{equation}
\frac{d\log e_{AB}}{d\tau}
=-\frac{N\rho_1^*+(N-2)\rho}{ND_N}<0 ;
\label{eq:N-slopes}
\end{equation}
\item[(ii)] the response against an untargeted bystander is
\begin{equation}
...
\label{eq:N-slopes-C}
\end{equation}
\item[(iii)] $A$'s effective exchange rate appreciates for every $N\geq2$ and every admissible $\rho$,
\begin{equation}
\frac{d\nu_A^E}{d\tau}
=\frac{\nu_B+(N-2)\nu_C}{N-1}
=-\frac{\rho_1^*}{D_N}<0 ;
\label{eq:N-neer}
\end{equation}
\item[(iv)] the bilateral bystander response reverses sign, so that $A$ depreciates against $C$, if and only if
\begin{equation}
\boxed{\;\rho>\rho_N^*\equiv N\rho_1^*\;}.
\label{eq:Nthresh}
\end{equation}
\end{enumerate}
\label{res:unilateral}
\end{theorem}

\noindent Parts (i)--(ii) follow from the target and bystander values of relative treatment, part (iii) from average protection alone, and part (iv) from the balance between the two.

\subsection{The role of $\rho$, $\eta$, and $N$}

\label{sec:roles}

The bilateral reversal threshold in \eqref{eq:Nthresh} differs from the condition under which the bystander's trade balance improves at unchanged exchange rates. From \eqref{eq:impact-chain},
\begin{equation}
...
The two elasticities enter at distinct margins. The elasticity $\eta$ governs substitution between domestic and imported tradables and enters only through $\rho_1^*$ and $D_N$; it is the sole substitution margin at $N=2$. The elasticity $\rho$ governs substitution across foreign origins and enters \emph{both} sides of \eqref{eq:adjust}: it raises the relative-treatment disturbance $g_D$ in \eqref{eq:g-eigen} and, through $D_N$, the adjustment coefficients $j_S,j_D$ in \eqref{eq:j-eigen}. The threshold $\rho_N^*$ is the point at which the first effect overtakes the average-protection disturbance, and $D_N>0$ scales both responses without affecting their signs.

The factor $N$ in \eqref{eq:Nthresh} comes entirely from the adjustment side. From \eqref{eq:g-eigen} and \eqref{eq:j-eigen},
\begin{equation}
...
\label{eq:gj-ratios}
\end{equation}
The relative strength of the relative-treatment and average-protection \emph{disturbances} is therefore the same at every $N$: trade diversion is not mechanically diluted as the number of partners grows, and \eqref{eq:F-general} confirms that the impact threshold $\rho>\rho_1^*$ for $F_C>0$ is itself independent of $N$. What changes with $N$ is the ease with which relative exchange-rate movements clear those disturbances. A common movement $\nu=\bar\nu\,\mathbf 1$ changes only the $N-1$ bilateral rates between $A$ and its partners, leaving the cross rates among partners unchanged; a zero-sum movement changes those cross rates as well. Under symmetry the additional margins make a relative movement exactly $N$ times as effective, so a relative disturbance requires only $1/N$ as much exchange-rate adjustment. Combining the two ratios, $\lambda_D/\lambda_S=(g_D/g_S)(j_S/j_D)=\rho/(N\rho_1^*)$, which is the content of \eqref{eq:Nthresh}.\footnote{The exact factor $N$ is specific to the symmetric nested-CES benchmark. Appendix~\ref{app:asym} considers asymmetric baselines and Appendix~\ref{app:het} heterogeneous substitution across origins.}
...
\begin{corollary}[Three-country illustration]
With $D_3>0$, Theorem~\ref{res:unilateral} at $N=3$ reads
\begin{equation}
\frac{d\log e_{AB}}{d\tau}
...
The competing-exporter result provides a useful comparison with the three-country case of our model. An improvement in $C$'s terms of trade corresponds to an increase in $e_{AC}$ by \eqref{eq:onetoone}. In the competing-exporter model this occurs unconditionally after a tariff on $B$, whereas in our model Corollary~\ref{res:3c} requires
\begin{equation*}
\rho>3\rho_1^*.
\end{equation*}
...
\label{eq:het-modes}
\end{equation}
and for $t\in V_r$ the response is $\nu=\lambda^{(r)}t$. Exchangeability is the case $\Gamma=\mathcal S_{N-1}$, which has exactly two components, $\operatorname{span}\{\mathbf 1\}$ and $\mathbf 1^\perp$, recovering Lemma~\ref{lem:exch} with $\lambda_S$ and $\lambda_D$.
\label{res:het-modes}
\end{proposition}
...
\begin{enumerate}
\item[(a)] Exchangeability of the partners (Lemma~\ref{lem:exch}): two modes, $\lambda_S$ and $\lambda_D$.
...
\begin{proof}[Proof of Theorem~\ref{res:adjust}(ii)]
...
\begin{proof}[Proof of Theorem~\ref{res:adjust}(i) and (iii)]
...
\begin{proof}[Proof of Corollary~\ref{res:3c}]
...
\begin{proof}[Proof of Theorem~\ref{res:unilateral}(i), (ii) and (iv)]
...
\begin{proof}[Proof of Theorem~\ref{res:unilateral}(iii)]
