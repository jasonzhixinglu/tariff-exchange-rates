...
``conventional'' exchange rate adjustment: surplus countries appreciate, deficit countries depreciate.

Given an adjustment mechanism satisfying the multilateral Marshall--Lerner condition, the tariff-induced trade-balance disturbances translate into exchange-rate movements with \emph{nonnegative weights}. Opposite-signed bilateral responses therefore require a mixed-sign $F$, i.e., bilateral ambiguity comes from the tariff disturbance on trade balances and not from the adjustment mechanism.\footnote{Ranking the notions: gross substitutability $\Rightarrow$ multilateral Marshall--Lerner. We assume the former as the primitive (Assumption \ref{as:gs}), but the subsequent analysis uses only \eqref{eq:invpos}. Under the Metzler sign structure of Assumption \ref{as:gs}, the eigenvalue statement in \eqref{eq:invpos} --- local stability of the tatonnement $\dot\nu_i = k_i \mathrm{TB}_i$, any $k_i > 0$ --- is equivalent to inverse positivity and $\det(-J) > 0$; absent that sign structure, stability alone is weaker and does not deliver the nonnegative inverse.}

Since $F$ is linear in $\rho$ and $\operatorname{adj}(-J)$ has degree $N-2$, each weighted sum in \eqref{eq:signrule} is a degree-$(N-1)$ polynomial in $\rho$: one relevant root at $N = 3$, possibly several sign changes at $N \geq 4$.
