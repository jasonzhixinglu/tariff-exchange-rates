...
Let $\widetilde J$ be the full $N \times N$ Jacobian before dropping country $A$. Homogeneity (which states that only relative prices enter \eqref{eq:dp}) and Walras' law give the identities
\begin{equation}
...
\noindent In words, the multilateral Marshall--Lerner condition says that the exchange-rate adjustment mechanism is regular in the sense of the two-country condition: each country's own depreciation improves its own trade balance, no country's appreciation worsens another country's balance, and the own effect dominates the cross effects. Any pattern of trade imbalances is then eliminated by conventional adjustment --- surplus countries appreciate, deficit countries depreciate --- and the mapping from disturbances to exchange-rate responses preserves signs.

Given an adjustment mechanism satisfying the multilateral Marshall--Lerner condition, the sign rule translates the tariff-induced trade-balance disturbances into exchange-rate movements with \emph{nonnegative weights}. Opposite-signed bilateral responses therefore require a mixed-sign $F$, i.e., bilateral ambiguity comes from the tariff disturbance on trade balances and not from the adjustment mechanism.

Three related notions appear above, ranked as: gross substitutability $\Rightarrow$ multilateral Marshall--Lerner $\Leftrightarrow$ local stability. Gross substitutability (Assumption \ref{as:gs}) is a restriction on primitives, sufficient but not necessary for \eqref{eq:invpos}. The multilateral Marshall--Lerner condition is the working condition; everything below uses only \eqref{eq:invpos}. Local stability is not a third requirement: the eigenvalue statement in \eqref{eq:invpos} is local stability of the tatonnement $\dot\nu_i = k_i \mathrm{TB}_i$ for any speeds $k_i > 0$, so stability, inverse positivity, and $\det(-J) > 0$ are equivalent faces of one condition. Since $F$ is linear in $\rho$ and $\operatorname{adj}(-J)$ has degree $N-2$, each weighted sum in \eqref{eq:signrule} is a degree-$(N-1)$ polynomial in $\rho$: one relevant root at $N = 3$, possibly several sign changes at $N \geq 4$.
...
The disturbance-adjustment representation \eqref{eq:system} and the multilateral Marshall--Lerner condition \eqref{eq:invpos} do not use symmetry; the exact thresholds of Section 3 and the NEER cancellation of Section 4 do. This appendix establishes what survives without it: the bilateral reversal region survives at any interior baseline, and the NEER result is preserved to leading order within a structured asymmetric family.
