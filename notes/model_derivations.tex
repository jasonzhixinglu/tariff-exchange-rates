...
\section{Disturbance and Adjustment: General Results}

Exchange-rate determination under the trade-balance closure has a structure that does not depend on the nested-CES specification. This section states the general results once; the next section computes their ingredients in the model.
...
Under the multilateral Marshall--Lerner condition, opposite-signed bilateral responses require a mixed-sign $F$, i.e., bilateral ambiguity comes from the tariff disturbance on trade balances and not from the adjustment mechanism.

\section{Tariff Responses in the Model: Two, Three, and $N$ Countries}

This section computes the entries of $J$ and $G$ for the nested-CES model and specializes to $N = 2$, $N = 3$, and general symmetric $N$.
...
\label{eq:dtb}
\end{align}
Reading the coefficients off \eqref{eq:dtb} gives the model's entries of $J$ and $G$ in \eqref{eq:JFdef}: the nested CES delivers specific parametric expressions for every term.
...
In the specializations below, $A$ imposes $d\tau = dt_{AB}$ at the symmetric baseline, and every threshold is measured in units of
\begin{equation}
\rho_1^*
