...
Define $e_{ij}$ = units of $i$'s currency per unit of $j$'s, $e_{ii} = 1$, $e_{ij} = e_{ik}e_{kj}$. \textbf{Therefore, a rise in $e_{ij}$ is a depreciation of $i$ against $j$}. Log perturbations in bilateral exchange rates relative to $A$ are defined by $\nu_j \equiv d\log e_{Aj}$, $\nu_A = 0$. Define relative wages, terms of trade, real exchange rates, and effective exchange rates:
\begin{equation}
