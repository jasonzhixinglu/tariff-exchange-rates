...
\]
The first margin determines the sign of the aggregate response of the tariffing country's currency. The second determines how the adjustment is distributed across individual foreign partners; through equilibrium it also affects the magnitude, though not the sign, of the aggregate response. The two-country model contains only the first observable margin, because there is only one foreign supplier. A multilateral model contains both. Bilateral exchange rates reflect both margins, whereas aggregation across trading partners largely removes the second. In this sense, the conventional two-country result is best understood as an aggregate result that happens also to be bilateral when there is only one foreign country.
...
The results sit at four levels of generality, and it is worth stating the hierarchy at the outset:
\begin{center}
\begin{tabular}{l}
\textbf{General:} any differentiable multilateral equilibrium separates disturbance $F$ from adjustment $J$ \\[2pt]
$\downarrow$ \\[2pt]
\textbf{Broad mechanism:} home-versus-foreign adjustment versus foreign-supplier reallocation \\[2pt]
$\downarrow$ \\[2pt]
\textbf{Nested CES:} $\eta$ and $\rho$ isolate the two margins with constant elasticities \\[2pt]
$\downarrow$ \\[2pt]
\textbf{Symmetric nested CES:} exact linear thresholds and the exact NEER cancellation \\
\end{tabular}
\end{center}
Each layer below inherits the logic of the layer above and adds structure. The exact formulas belong to the bottom layer; the mechanism they illustrate belongs to the layers above it.
...
The distinction between the two margins is more general than the particular CES specification used here. Any demand system in which total import expenditure and its allocation across foreign origins can adjust separately contains analogous margins. The nested CES structure is useful because it isolates them with two constant elasticities, $\eta$ and $\rho$, and thereby yields closed-form thresholds. The exact comparative statics derived below, including the linear thresholds in $\rho$ and the exact cancellation in the effective exchange rate, should therefore be understood as results of the nested-CES environment rather than consequences of the conceptual distinction alone. The mechanism is general; the parameterization and the exact theorems are model-specific.
...
This is not a failure of Marshall--Lerner logic. The ambiguity arises because the tariff creates a mixed pattern of trade-balance disturbances across countries. The lesson is more general than the factor of three: \emph{a bystander's own favorable trade-diversion shock need not determine its equilibrium bilateral exchange rate, because that rate responds to the entire vector of trade-balance disturbances through $(-J)^{-1}F$}. That observation motivates a general way to organize the comparative statics, introduced next.
...
The decomposition itself is the most general layer of the analysis. Any differentiable multilateral trade environment with $N-1$ independent trade-balance conditions admits a local representation of the form $J\, d\nu + G\, d\boldsymbol{\tau} = 0$, and the disturbance-adjustment reading does not rely on the symmetry used for the closed-form results. The nested CES structure enters at a lower level: it supplies the explicit expressions for $J$ and $G$ derived below, and primitive conditions for their signs. The general representation and the particular formulas should be kept distinct.

\subsection{General linearization}

The equations of this subsection are the nested-CES expressions for the general objects: they compute the entries of $J$ and $G$ for the demand system of Section 2. Evaluate at $\tau_{ij} = 0$ with balanced trade, and normalize baseline incomes. The baseline objects are the home share within tradables $h_i = \alpha_D$, the within-import shares $b_{ij} = \beta_{ij}$, the income shares $s_{ij} = \alpha_T(1-\alpha_D) b_{ij} = m\, b_{ij}$, and the flow values $f_{ij} = s_{ij} I_i$ from \eqref{eq:tb} at $\tau = 0$. A country-specific home share $h_i$ carries through the derivation unchanged. Balance requires $\sum_k f_{ki} = \sum_j f_{ij}$.
...
\noindent Two qualifications keep the statement precise. The exact cancellation behind it is a property of the symmetric nested-CES environment (Sections 2.2 and 10); the decomposition of margins is more general. And $\rho$ does not disappear from the NEER response: it cancels from the numerator, but remains in $D$, so foreign-supplier substitution affects the magnitude of the aggregate adjustment even though, under symmetry, it cannot affect its sign.

\paragraph{Where the conditions sit empirically.} Given \eqref{eq:invpos}, $D > 0$ and $\rho_1^* > 0$ hold at every admissible calibration. The signs of $e_{AB}$, $e_{BC}$, and the NEER are therefore parameter-free. The one genuinely ambiguous object is the bilateral rate against the bystander, and locating its threshold empirically requires care. The parameter $\rho$ is a specific structural object: substitution across foreign origins conditional on the import composite, in this two-layer demand system. Estimates labeled ``trade elasticities'' in the literature are not, in general, estimates of that object; they mix home-versus-foreign substitution, extensive margins, intermediate inputs, and gravity responses. What can be said is qualitative. Estimated upper-nest (home-versus-import) elasticities are typically low, and estimated lower-nest (cross-origin) elasticities are typically several times larger, so a benchmark threshold of $\rho_3^* = 3.96$ is neither clearly above nor clearly below plausible values of $\rho$. Neither sign of $d\log e_{AC}$ can be ruled out, but a sharper statement would require estimates that map onto $\rho$ directly. The trade-war threshold of Section 8, $\rho_1^* = 1.32$, is less sensitive to this mapping: it asks only that foreign origins substitute more easily with one another than the level at which home and foreign goods are usually estimated to substitute, so the war predictions are comparatively robust.
...
\end{equation}
The threshold rises with $N$ because diverted expenditure splits across $N-2$ bystanders, each receiving weight $1/(N-1)$, so a larger $\rho$ is needed to reverse any one bilateral rate. This is a statement about how the adjustment is distributed across partners, not about the aggregate value of $A$'s currency.
\end{result}

The economic force behind the rising threshold is dilution. With one bystander, expenditure diverted away from $B$ has only one alternative foreign destination. With $N-2$ bystanders, the same foreign reallocation is spread across many suppliers. Each individual bystander receives a smaller diversion gain, and stronger cross-origin substitutability is required to generate enough appreciation to reverse its bilateral rate against $A$. Dilution is a force one can expect in many models; the exact linearity of the threshold in $N$ is a property of the symmetric nested-CES structure.
...
Under the symmetric nested-CES structure the decomposition becomes especially sharp: the home-versus-foreign component determines the sign of the effective response, while cross-origin reallocation governs the dispersion of bilateral responses. The statement is about signs, not orthogonality. Foreign-supplier substitution determines the bilateral distribution of the adjustment and also affects the magnitude of the aggregate response through $D_N$; under symmetry it cannot affect the aggregate sign. Its qualitatively new role is to redistribute the adjustment across foreign partners.
...
To leading order, asymmetry simply scales the symmetric effective appreciation by the relative importance of the tariff target. Within this structured class of asymmetric economies, the symmetric result is therefore not knife-edge, even though its exact finite-$N$ cancellation is. The experiment does not establish robustness to arbitrary asymmetry; it establishes it for the $\kappa$-parameterized family.
...
\end{itemize}
The exact cancellation of \eqref{eq:N-neer} is therefore one-sided. Tariffs on disproportionately large partners preserve the conventional aggregate sign for every $\rho$. Tariffs on minor partners can reverse it, but only at $\rho$ of order $N$. The same dilution that makes individual bilateral reversals harder as $N$ grows also makes aggregate reversal under asymmetry difficult: such values lie far above the war threshold $\rho_1^*$ and, at moderate $N$, above plausible elasticities ($\rho_E^* \approx 22$ at $N = 10$, $\kappa = 0.5$, benchmark parameters). The symmetric result should be read as exact under symmetry and approximately robust within the $\kappa$-parameterized class, rather than as a knife-edge curiosity; robustness to arbitrary asymmetric trade networks is not claimed.

\section{Why the Canonical PTA Model Is Sharper}

The multilateral model predicts that an untreated exporter benefits from a discriminatory tariff only when cross-origin substitution is sufficiently strong. This may initially appear to conflict with the canonical theory of preferential trade agreements, in which discrimination against one exporter unambiguously improves the terms of trade of its competitor. The difference is not a competing mechanism. It is a difference in the set of margins permitted by the two environments. The right framing is therefore not that nested CES produces a threshold where Viner--Mundell does not. It is that the nested-CES model provides a tractable way of placing several economically meaningful margins in one environment, and the threshold states how strong foreign-supplier substitution must be to overcome the opposing margins. This section also shows that the threshold is not an artifact of CES curvature: each of its pieces corresponds to an identifiable economic force absent from the canonical setup.
...
\]
The first margin determines the sign of the aggregate response of the currency. The second determines the distribution of that adjustment across bilateral exchange rates, and through equilibrium it also affects the magnitude of the aggregate response. The results sit in the hierarchy stated in the Introduction: the disturbance-adjustment logic is general, the two-margin mechanism is broad, the nested CES isolates the margins with two elasticities, and symmetry delivers the exact thresholds and the exact cancellation. Distinguishing these layers clarifies both what survives from the two-country model and what genuinely changes in a multilateral world.
