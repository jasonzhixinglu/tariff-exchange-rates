...
\begin{result}
The canonical PTA model is the restriction of the three-country model to a subspace on which $\rho_3^* \leq 0$: the unconditional Viner--Mundell sign is the threshold condition evaluated where it cannot bind. The nested model embeds the same diversion mechanism among the opposing margins, and $\rho > \rho_3^*$ states when it dominates.\footnote{Analytical PTA models obtain sharp signs by such restrictions (competing exporters: Bagwell--Staiger 1999, Ornelas 2005; symmetric endowment blocs: Krugman 1991, Bond--Syropoulos 1996; Viner: $\rho = \infty$; Kemp--Wan 1976: no restrictions, no sign claims). Applied GTAP-class and quantitative trade models retain every margin and generate both signs in simulations without stating the separating condition. Appendix C verifies the decomposition by recomputing the threshold with each margin removed.}
\label{res:pta}
\end{result}
...
\section{Threshold decomposition: restriction table}

Recomputing $\rho_3^*$ with each margin of \eqref{eq:pta-decomp} removed (severing trade changes the baseline, so the pieces are approximately additive):
...
\noindent The largest piece is the $B$--$C$ linkage; two routes reach $\rho_3^* \leq 0$, and the canonical PTA model takes both.
