...
Figure~\ref{fig:results} reports model-implied Regime-2 exchange rate changes across the $\rho$ grid alongside realized data. The data refer to percent changes\footnote{We report $100 \times (\text{level}_t / \text{level}_0 - 1)$ relative to the 2024 annual average.} in observed bilateral rates relative to the 2024 annual average\rev{, at two horizons: the April 2025 monthly average (the peak-escalation month) and, because the model's object is a long-run position that becomes visible only as the transition plays out, the December 2025 monthly average as an extended window. Throughout, a positive change denotes a depreciation of the first-named currency: $\Delta e_{AC} > 0$ means the dollar depreciates against $C$'s currency. For the ROW configuration we construct a \emph{matched} comparison index: the model's $C$ is the world excluding the US and China, so we strip the renminbi component out of the Federal Reserve H.10 broad nominal dollar index using the Fed's published weight ($10.897$ percent) and renormalize, $\Delta \log I^{\text{ex-CN}} = (\Delta \log I^{\text{broad}} - w_{\text{CN}} \Delta \log e_{\text{CNY/USD}})/(1 - w_{\text{CN}})$. This replaces the advanced-economy index used in an earlier version, which excluded emerging markets from the aggregate the model actually speaks to --- and the replacement matters: the matched index shows the dollar \emph{appreciating} slightly against the ex-China world in April 2025 ($-0.95$ percent), where the AE index showed depreciation.}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{calibration_nested_results.pdf}
\caption{\rev{Regime-2 model predictions across the cross-origin elasticity grid, against data at two horizons. Solid: model $\Delta e_{AC}$ (USD against $C$'s currency); dashed: model $\Delta e_{AB}$ (USD/RMB); horizontal lines: observed changes relative to the 2024 average at the April 2025 (dotted) and December 2025 (dash-dotted) windows, colored to match the corresponding model object. Shaded band: the empirically central cross-origin range $\rho \in [2, 4]$. Vertical dotted line: the configuration's cumulative reversal threshold at $\tau = 1.45$. Positive values denote dollar depreciation. The Vietnam panel is a single-bystander upper bound (see text).}}
\label{fig:results}
\end{figure}

\begin{revblock}
Consider first the EU configuration, the cleanest test because the euro is a large, floating currency. At $\rho = 2.0$, the model's Regime-2 prediction is $(\Delta e_{AB}, \Delta e_{AC}) = (-0.80, +3.45)$ percent against April 2025 data of $(-1.51, +3.36)$: both signs and both magnitudes. At $\rho = 2.6$ --- the value implied by the \cite{liqiu2022} elasticity ratio at our $\eta$ --- the prediction is $(+0.29, +9.44)$ against December 2025 data of $(+1.94, +8.16)$. A single cross-origin elasticity in the center of the empirically supported band thus rationalizes the euro--dollar--renminbi triangle at \emph{both} horizons in sequence: the near-term response undershoots the long-run prediction, and eight months later the data have moved most of the remaining distance toward it. We do not lean hard on the December comparison as a point test --- the extended window reflects partial de-escalation of the bilateral tariffs after May 2025 as well as continued adjustment, so it is best read qualitatively: the transition moved \emph{toward} the model's long-run position, not away from it.

The ROW configuration, evaluated against the matched ex-China index, tells a consistent story with an honest wrinkle. At $\rho = 3.0$ the model predicts $(\Delta e_{AB}, \Delta e_{AC}) = (-3.78, -1.34)$ against April data of $(-1.51, -0.95)$: in April the dollar \emph{appreciated} slightly against the ex-China world, and the model reproduces this --- at moderate $\rho$ the asymmetric war ($\tau_{AB} > \tau_{BA}$, plus the $10$ percent tariff on $C$ itself) leaves the dollar mildly strong against a large bystander. By December the matched index had swung to $+2.27$ percent, which the model reaches only at high $\rho$; at central values it predicts a smaller depreciation, and the December renminbi appreciation ($+1.94$) is outside the Regime-2 predictions entirely --- unsurprising, again, given post-May de-escalation. The Vietnam configuration is reported as a mechanism illustration rather than a quantitative test: with a single bystander one-eighteenth the size of the US, the model routes the entire diversion of the trade war through one small economy, an upper bound by construction (the figure's scale differs by an order of magnitude), and the dong's persistent managed appreciation of the dollar in the data ($-3.06$ percent in April, $-4.83$ by December) reflects exactly the financial and policy channels the model excludes --- in the configuration where Section~\ref{sec:impossibility} says the trade channel's imprint on the bilateral rate is weakest and the reversal threshold highest.
